#!/usr/bin/env bash
# Install and configure MySQL on ubuntu machine as a Replication source
# Run it with the sudo comand or under root user
# Ubuntu relaise: Ubuntu 20.04.5 LTS
# MySQL Version: 5.7.37

#########################
#    Install MySQL
#########################
# setting the PGP key for the apt package manager
echo "-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: SKS 1.1.6
Comment: Hostname: pgp.mit.edu


sTSKHe+QnnnoFmu4gnmDU31i
=Xqbo
-----END PGP PUBLIC KEY BLOCK-----" > signature.key
# add mysql PGP key to apt-key list
#apt-key add signature.key
# add the mysql repo to apt repo list
#sh -c 'echo "deb http://repo.mysql.com/apt/ubuntu bionic mysql-5.7" >> /etc/apt/sources.list.d/mysql.list'
# import the public key for signature verification
#apt-key adv --keyserver keyserver.ubuntu.com --recv-keys B7B3B788A8D3785C
# update the apt list
#apt-get update
# install mysql version 5.7
#apt-get install -f -y mysql-client=5.7* mysql-community-server=5.7* mysql-server=5.7*
rm -f signature.key

###########################################
#  configure replication source (master)
###########################################
mysql_conf='/etc/mysql/mysql.conf.d/mysqld.cnf'

# rewrite the bind-address directive
#check if the bind-address directive exists and commented out
grep -s 'bind-address' "$mysql_conf"
#rewrite it if it exists
if [ "$?" -eq 0 ]
then
    sed -in 's/^#*\s*bind-address\s\+=\s*.*/#bind-address      = 54.84.73.143/' "$mysql_conf"
    echo "=========REWRITe bind-address=============="
else
    #append it to the end of the file
    echo "bind-address        = 54.84.73.143" >> "$mysql_conf"
fi

# rewrite the server-id directive
grep -s 'server-id' "$mysql_conf"
#rewrite it if it exists
if [ "$?" -eq 0 ]
then
    sed -in 's/^#*\s*server-id\s\+=\s*.*/server-id      = 7/' "$mysql_conf"
    echo "=========REWRITe server-id=============="
else
    #append it to the end of the file
    echo "server-id        = 7" >> "$mysql_conf"
fi

# rewrite the log-bin directive
grep -s 'log_bin' "$mysql_conf"
#rewrite it if it exists
if [ "$?" -eq 0 ]
then
    sed -in 's/^#*\s*log_bin\s\+=\s*.*/log_bin      = \/var\/log\/mysql\/mysql-bin.log/' "$mysql_conf"
    echo "=========REWRITe log_bin=============="
else
    #append it to the end of the file
    echo "log_bin        = /var/log/mysql/mysql-bin.log" >> "$mysql_conf"
fi

# rewrite the binlog_db directive
grep -s 'binlog_do_db' "$mysql_conf"
#rewrite it if it exists
if [ "$?" -eq 0 ]
then
    sed -in 's/^#*\s*binlog_do_db\s\+=\s*.*/binlog_do_db      = tyrell_corp/' "$mysql_conf"
    echo "=========REWRITe binlog_do_db=============="
else
    #append it to the end of the file
    echo "binlog_do_db        = tyrell_corp" >> "$mysql_conf"
fi

# restart mysql server to reaload the configurations
systemctl restart mysql
############################################
#   configure firewall for mysql port
############################################
#ufw allow from 35.175.102.250 to any port 3306
