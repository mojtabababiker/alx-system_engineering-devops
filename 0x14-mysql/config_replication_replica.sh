...
# rewrite the relay-log directive
#check if the relay-log directive exists and commented out
grep -s 'relay-log' "$mysql_conf"
#rewrite it if it exists
if [ "$?" -eq 0 ]
then
    sed -in 's/^#*\s*relay-log\s\+=\s*.*/relay-log      = \/var\/log\/mysql\/mysql-relay-bin.log/' "$mysql_conf"
    echo "=========REWRITe relay-log=============="
else
...
if [ "$?" -eq 0 ]
then
    sed -in 's/^#*\s*server-id\s\+=\s*.*/server-id      = 13/' "$mysql_conf"
    echo "=========REWRITe server-id=============="
else
    #append it to the end of the file
    echo "server-id        = 13" >> "$mysql_conf"
fi
